#!/usr/bin/env python3
"""Build HHH user-manual locale decks from EN (translate + soft VO).

Locales mirror app i18n: de, es, fr, it, pt, zh, hi.
"""
...
EN_DIR = ROOT / "videos" / "user-guide-hhh"
EN_NARR = EN_DIR / "narration-en.json"

LOCALES = {
    "de": ("de", "de-DE-ConradNeural", "Handy Horology Helper Benutzerhandbuch"),
    "es": ("es", "es-US-AlonsoNeural", "Manual de usuario de Handy Horology Helper"),
    "fr": ("fr", "fr-FR-DeniseNeural", "Manuel utilisateur Handy Horology Helper"),
    "it": ("it", "it-IT-ElsaNeural", "Manuale utente Handy Horology Helper"),
    "pt": ("pt", "pt-BR-FranciscaNeural", "Manual do usuário Handy Horology Helper"),
    "zh": ("zh-CN", "zh-CN-YunyangNeural", "Handy Horology Helper 用户手册"),
    "hi": ("hi", "hi-IN-SwaraNeural", "Handy Horology Helper उपयोगकर्ता मैनुअल"),
}
...
def translate_batch(texts: list[str], target: str) -> list[str]:
    """Translate with small chunks to avoid Google length limits."""
    out: list[str] = []
...
def patch_html(src_html: str, lang: str, title: str, narr: list[str], code: str) -> str:
    ...
    html = re.sub(
        r"<h1>Handy Horology Helper User Manual</h1>",
        f"<h1>{title}</h1>",
        html,
        count=1,
    )
    ...
    # Language picker: clear EN current, mark this locale
    html = html.replace('class="is-current" href="/videos/user-guide-hhh/"', 'href="/videos/user-guide-hhh/"')
    html = html.replace(
        f'href="/videos/user-guide-hhh-{code}/"',
        f'class="is-current" href="/videos/user-guide-hhh-{code}/"',
        1,
    )
    # Point relative audio stays ./audio/slide-N.mp3 (same structure)
    return html


def build_locale(code: str, force_audio: bool, skip_translate: bool) -> None:
    gt_code, voice, title = LOCALES[code]
    out_dir = ROOT / "videos" / f"user-guide-hhh-{code}"
    narr_path = out_dir / f"narration-{code}.json"
    audio_dir = out_dir / "audio"

    out_dir.mkdir(parents=True, exist_ok=True)
    # Copy player shell + css
    ...
    if skip_translate and narr_path.exists():
        ...
        # Preserve proper shelf prices in every language
        for i, t in enumerate(texts):
            # Ensure dollar amounts survive translation quirks
