...
SLIDES: list[dict] = [
    # —— Install (0–3) ——
    T("Welcome to Pocket Allowance Ledger — PAL for short. This interactive user guide walks through every major parent and kid-web flow using the pre-seeded Reyes demo household.", ICON, "PAL app icon"),
    T("On iPhone, install through TestFlight. Accept the invite email, open the TestFlight app, and tap Install next to Pocket Allowance Ledger.", ICON, "TestFlight install"),
    T("On Android, accept the Google Play Internal testing invite. Open the Play Store testing link and install Pocket Allowance Ledger from there.", ICON, "Play Internal install"),
    ...
    # —— Reyes demo (4–8) ——
    T("Open PAL. The Reyes demo household is already loaded — no setup wizard. Family name Reyes. Kids Maya, about eight, and Jacob, about six.", HOME, "Home — Reyes demo"),
    T("Family password for kid web is reyes — lowercase. Kid PIN for both children is one-two-three-four. These credentials match Settings → Reviewer guide.", SET, "Reviewer guide credentials", tap(50, 72, "Reviewer guide", 1.4, 2.6)),
    ...
    T("Suggested evaluation time is twenty-five to forty minutes. Follow the chapter pills to jump ahead, or play straight through with synced narration.", ICON, "PAL"),
    T("This build is marketing zero point one dot two — full-featured demo with no paid subscriptions. PocketBudJet expense bridges and cloud sync are out of scope.", SET, "Settings"),
    # —— Reviewer guide (9–13) ——
    T("Open Settings from the gear icon or the Settings tab. Scroll to Reviewer guide for the full credential card and suggested walkthrough.", SET, "Settings", tap(50, 88, "Settings", 0.8, 2.4)),
    T("Reviewer guide lists the demo household, install path, suggested steps, fidelity evaluation points, and what is explicitly not in this build.", SET, "Reviewer guide", tap(50, 55, "Reviewer guide", 1.0, 2.8)),
    T("Step one on the guide: Home — tap Done on a contract and confirm the coin lands in Balances.", HOME, "Home Done buttons", tap(50, 54, "Done", 1.0, 2.8)),
    ...
    T("Young contracts may show an extinction-burst callout on Home — coaching copy when behavior temporarily worsens after thinning. Evaluate whether the wording feels literature-first.", HOME_QR, "Extinction burst callout", tap(50, 38, "Callout", 1.2, 2.6)),
    ...
    T("Reviewer guide includes an Open Kid web link button that jumps straight to this screen.", SET, "Kid web screen"),
    ...
    T("First-week versus now comparison cards highlight behavior change over time — useful for fidelity evaluation.", REP, "First week vs now", tap(50, 48, "Insights", 1.0, 2.6)),
    ...
    T("Coaching cards use literature-first language. Evaluate whether wording could be misused by a stressed parent — concrete fidelity notes welcome.", REP, "Coaching card"),
    ...
    T("Export from Settings if you need a backup zip for evaluation — no secrets are included in the archive.", REP, "Reports overview"),
    # —— Contracts (54–61) ——
    ...
    T("Extinction-burst callouts on young contracts explain temporary behavior spikes after schedule changes — evaluate copy fidelity here.", CON_DET, "Extinction burst coaching"),
    ...
    T("Help links to support email and this user guide. About shows version and fidelity boundary notes.", SET, "Help and About"),
    T("Export backup creates a local zip of contracts, tokens, and settings — useful for reviewer evaluation. No cloud sync in v1.", SET, "Backup export", tap(50, 78, "Export", 1.6, 2.4)),
    T("Restore from backup on a fresh install if you need to reset the Reyes demo mid-review.", SET, "Restore backup"),
    T("Kid web link in Reviewer guide opens the LAN URL screen directly — handy when testing with Hank or other reviewers.", SET, "Open Kid web link", tap(50, 68, "Kid web", 1.2, 2.4)),
    # —— Fidelity (68–73) ——
    T("Evaluate fidelity of token-economy language versus chore-tracker UX. PAL avoids gamified streaks and shame metrics.", HOME, "Fidelity — language"),
    T("Schedule thinning and saving cadence appear as age priors plus performance nudges plus parent confirm — literature-first with socioeconomic flexibility for reinforcer delivery.", REP, "Fidelity — thinning"),
    T("Delayed verification path should feel trustworthy: kid reports, parent confirms, tokens land. Notice timing and wording parents might misuse.", APP, "Fidelity — verification"),
    T("Extinction-burst coaching on Home and Contracts — does the copy help parents stay consistent without sounding clinical?", HOME_QR, "Fidelity — extinction burst"),
    T("Would you trust this with a real family after copy polish? Concrete fidelity notes beat feature wishlists.", SET, "Fidelity — trust"),
    T("Literature-first with socioeconomic flexibility when household means require different reinforcer delivery. Age bands are developmental defaults only.", SET, "Fidelity boundary"),
    # —— Out of scope (74–76) ——
    T("Explicitly not in this build: debit or banking, PAL Coach chatbot, cloud sync, ages under six, PocketBudJet expense bridge, and paid subscriptions.", SET, "Out of scope list"),
    ...
    T("Feedback preferred at support at josspatech dot com. Named reviewers are not listed as product endorsers until they opt in after review.", ICON, "Feedback"),
    # —— Thank you (77) ——
    T("Thank you for reviewing Pocket Allowance Ledger. This guide lives at josspatech dot com slash videos slash pal slash user-guide. Tap any chapter pill to re-listen a section.", ICON, "Thank you"),
]
...
<head>
  ...
  <meta name="description" content="Interactive PAL user guide — TestFlight install, Reyes demo household, kid web on home Wi‑Fi, parent core flow through Contracts. {len(SLIDES)} slides with synced narration.">
  ...
  <meta property="og:description" content="Interactive PAL user guide — install, demo household, kid web, Approvals, Reports, and Contracts with synced narration.">
...
<body data-jt-product="pal">
  ...
  <div class="hero">
    <div class="container">
      ...
      <p class="subheader">{len(SLIDES)} interactive slides with synced narration, gold tap guides, and chapter jumps. Pre-seeded Reyes demo — TestFlight or Play Internal, parent app plus kid web on home Wi‑Fi.</p>
    </div>
  </div>
  <section class="user-manual walkthrough">
    <div class="container">
      <h2>Interactive User Guide</h2>
      <p class="section-sub">{len(SLIDES)} slides with synced narration and gold tap guides — install through Contracts, kid web, and fidelity evaluation. Use the {len(CHAPTERS)} chapter pills to jump ahead, Previous/Next (or ← →) to re-listen a step, or tap any sentence in the transcript.</p>
...
{slides_html}
  ...
  <section class="cta-section">
    <div class="container">
      <h2>Ready to evaluate PAL with a real family lens?</h2>
      <p>Email support@josspatech.com for TestFlight or Play Internal access. Prefer concrete fidelity notes — wording, timing, what a parent might misuse — over feature wishlists.</p>
      <a href="mailto:support@josspatech.com?subject=PAL%20reviewer%20access" class="download-button">Request access</a>
